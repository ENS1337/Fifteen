    ...
    ui(new Ui::MainWindow)
{
	...
	moves_number = 0;
    collected_tiles = 0;
    ui->label->setText("Плиток собрано: " + QString::number(collected_tiles));
...
void MainWindow::Generate_Tiles(bool random) // функция для рандомного создания плиток
{
	...
	for (int i = 0; i < 4; i++)
	{
		...
		for (int j = 0; j < 4; j++,z++)
		{
            ...
            label->setStyleSheet("background-color: black;"); // делаем плитки черными
...
bool MainWindow::has_won() // функция, которая проверяет, расставлены ли плитки по порядку(выиграл ли игрок)
{
    ...
    collected_tiles=0;
   for (int i = 0; i < 4; i++) //Подсчет Кол-ва собранных плиток по порядку
    {
        for (int j = 0; j < 4; j++, z++)
        {
            if (z >= 15) break;
   ...
   z=0;
    ...
    z=0;
    if (is_winner == true) // При победе игрока, плитки перекрашиваются в зеленый
    {
        for (int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++, z++)
            {
                ...
                if (arr[i][j] != NULL)
                {
                    arr[i][j]->setStyleSheet("background-color: green;");
                }
            }
        }
    }
...
void MainWindow::processButton() // по нажатию плитки выполняется эта функция
{
	if (can_move)
	{
        ...
        int i = 0, j = 0;  // ищем индекс (на каком столбце и строке стоит плитка в матрице)
